  ...
  return (
    <Html center>
      {placeholderSrc ? (
        ...
        <div style={{ color: 'white', fontFamily: 'monospace' }}>{Math.round(progress)} %</div>
      )}
    </Html>
  );
};
...
const ModelInner = ({
  ...
  useLayoutEffect(() => {
    ...
    let s = 1;
    if (sphere.radius > 0 && sphere.radius !== Infinity) {
      s = 1 / (sphere.radius * 2);
    }
    g.position.set(-sphere.center.x + xOff, -sphere.center.y + yOff, -sphere.center.z);
    g.scale.setScalar(s);
    ...
    if (autoFrame && (camera as THREE.PerspectiveCamera).isPerspectiveCamera) {
      ...
      const d = (fitR * zoomMultiplier) / Math.sin((persp.fov * Math.PI) / 180 / 2);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ContactShadows ref={contactRef} position={[0, -0.5, 0]} opacity={0.35} scale={10} blur={2} frames={1} />
